...
public class PageRankReducer extends Reducer<Text, Text, Text, DoubleWritable> {
	...
	/*
	 * This is the PageRankReducer reducer task that gets from the mapper, for every row,
	...
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		...
		for (Text value : values) {
			...
			if (con != 0.0)
				contrib += (map.get(link[0])) / con;
		}
		double rank = (randomness / count) + ((1 - randomness) * (contrib + (dangling / count)));
